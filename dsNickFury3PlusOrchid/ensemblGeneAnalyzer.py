...
class CheckArgs():  #class that checks arguments and ultimately returns a validated set of arguments to the main program
    ...
    def __init__(self):
        ...
        parser.add_argument("-c", "--chromosome", help = "Chromosome.", default = "unknown", type = str)
        parser.add_argument("-f", "--writeToFile", help = "Write targets to a file rather than to the Azure table", action = "store_true")
        parser.add_argument("-o", "--outputFile", help = "Output file for raw output, ignored if writeToFile is not passed", default = None)
        ...
        self.writeToFile = rawArgs.writeToFile
...
class JobList(object):
    ...
    def __init__(self, targets, jobName, azureAccount, tableName, mock=False):
        import re
        self.targets = targets
        self.azureAccount = azureAccount
        self.tableName = tableName
        self.jobName = re.sub('\W', "_", jobName)
        self.createResultsDirectory(mock)
    ...
    def createJobBashLines(self):
        ...
        #    targetGroupMember += 1
            ...
            if to_azure:
                ...
                result_filename = self.geneResultsDir + os.sep + "_".join(azureTableInfo) + ".tab"
                bashLine = pythonInterpreterAbsolutePath + " dsNickFury3.3.py -m search -g hg38 -s " + self.targets[i][0].upper() + " -p NGG --azimuthSequence " + self.targets[i][1] + " --endClip 3 --outputToFile " + result_filename
    ...
    def createResultsDirectory(self, mock):
        ...
        if not mock:
            ...
            self.commandsDir = settings.pj(settings.commands_dir, args.chromosome)
            try:
                os.mkdir(self.resultsDir)
            except FileExistsError as e:
                pass

            self.geneResultsDir = os.path.join(self.resultsDir, args.chromosome, self.jobName)
            try:
...
def main():
    ...
    if args.writeToFile:
        import sys
        handle = open(args.outputFile, 'w') if args.outputFile else sys.stdout
        handle.write('transcript\ttarget\tbeforeAltStart\tlastExon\n')
        for ts, tlist in transcriptTargetData.items():
            for target in tlist:
                handle.write('{}\t{}\n'.format(ts, '\t'.join(map(str, target))))
        if handle is not sys.stdout: handle.close()
    else:
        azureAccount = settings.azure_account
        tableName = settings.table_name
        jobList = JobList(targetSiteSet, ensg, azureAccount, tableName, mock = args.mock)
        geneTranscriptData = GeneTranscriptData(transcriptTargetData, ensg)
        print("Submitting Azure Table")
        geneTranscriptData.submitAzureTable(azureAccount, tableName)
        if targetSiteSet:
